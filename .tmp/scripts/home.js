...
(function (exports) {
  ...
  var FULLSIZED_ICON = 336 * (window.devicePixelRatio || 1);
  var DEFAULT_ICON = 'url("/style/images/appic_developer.png")';
  var DEFAULT_BGCOLOR = 'rgba(0, 0, 0, 0.5)';
  var DEFAULT_BGCOLOR_ARRAY = [0, 0, 0, 0.5];
  var CARDLIST_LEFT_MARGIN = 8.4;
  ...
  Home.prototype = Object.defineProperties({
    navigableIds: ['search-button', 'search-input', 'edit-button', 'settings-button', 'filter-tab-group'],

    topElementIds: ['search-button', 'search-input', 'edit-button', 'settings-button'],

    bottomElementIds: ['filter-tab-group', 'filter-all-button', 'filter-tv-button', 'filter-device-button', 'filter-app-button', 'filter-website-button'],
    ...
    filterElementIds: ['filter-all-button', 'filter-tv-button', 'filter-device-button', 'filter-app-button', 'filter-website-button'],
    ...
    init: function init() {
      ...
      this.cardManager.getCardList().then(function (cardList) {
        ...
        that.cardListElem.addEventListener('transitionend', that.determineFolderExpand.bind(that));

        that.cardManager.on('card-inserted', that.onCardInserted.bind(that, that.cardScrollable));
        that.cardManager.on('card-removed', that.onCardRemoved.bind(that, that.cardScrollable));
        ...
        that.edit = new Edit();
        that.edit.init(that.spatialNavigator, that.cardManager, that.cardScrollable, that.folderScrollable);
        ...
        // handle animation
        that.endBubble = null;
        document.addEventListener('visibilitychange', that.onVisibilityChange.bind(that));
        ...
        cardList.forEach(function (card) {
          if (card instanceof Folder) {
            card.on('card-inserted', that.onCardInserted.bind(that, that.folderScrollable));
            card.on('card-removed', that.onCardRemoved.bind(that, that.folderScrollable));
          }
        });
    ...
    onVisibilityChange: function onVisibilityChange() {
      if (document.visibilityState === 'visible') {
        ...
        Promise.all([new Promise(function (resolve) {
          that.skipBubble = Animations.doBubbleAnimation(that.cardListElem, '.app-button', 100, resolve);
        }), new Promise(function (resolve) {
          if (that._folderCard) {
            that.skipFolderBubble = Animations.doBubbleAnimation(that.folderListElem, '.app-button', 100, resolve);
          } else {
            resolve();
          }
        })]).then(function () {
    ...
    initClock: function initClock() {
      ...
      navigator.mozL10n.ready(function () {
        ...
        window.addEventListener('moztimechange', that.restartClock.bind(that));
        // Listen to 'timeformatchange'
        window.addEventListener('timeformatchange', that.restartClock.bind(that));
      });
    },

    _localizeCardName: function _localizeCardName(elem, card) {
      if (!elem || !card) {
        return;
      }
    ...
    onCardInserted: function onCardInserted(scrollable, card, idx, overFolder) {
      if (card instanceof Folder) {
        card.on('card-inserted', this.onCardInserted.bind(this, this.folderScrollable));
        card.on('card-removed', this.onCardRemoved.bind(this, this.folderScrollable));
      }
      ...
      scrollable.on('slideEnd', (function () {
        ...
        if (scrollable.nodes.indexOf(newCardElem) === scrollable.nodes.length - 1) {
    ...
    onCardUpdated: function onCardUpdated(card, idx) {
      var that = this;
      var cardButton = this.cardScrollable.getItemFromNode(this.cardScrollable.getNode(idx));
      var spans = SharedUtils.nodeListToArray(cardButton.getElementsByTagName('span'));
    ...
    onCardRemoved: function onCardRemoved(scrollable, indices) {
    ...
    _setCardIcon: function _setCardIcon(cardButton, card, blob, bgColor) {
      try {
    ...
    _createWave: function _createWave(cardButton, card) {
    ...
    _fillCardIcon: function _fillCardIcon(cardButton, card) {
      var manifestURL = card.nativeApp && card.nativeApp.manifestURL;
      var that = this;
      // We have thumbnail which is created by pin
      if (card.thumbnail) {
        this._setCardIcon(cardButton, card, card.thumbnail, card.backgroundColor);
        // TODO add backgroundColor??? How to do it???
            ...
            if (size >= FULLSIZED_ICON) {
              ...
              that._getIconColor(blob, function (color, err) {
                if (err) {
                  ...
                  that._setCardIcon(cardButton, card, blob, 'rgba(' + color[0] + ', ' + color[1] + ', ' + color[2] + ', ' + color[3] + ')');
                }
              });
            }
          ...
          this._setCardIcon(cardButton, card, card.cachedIconBlob, card.backgroundColor);
        }
    },

    _getIconColor: function _getIconColor(blob, callback) {
      var dy = 0;
    ...
    onArrangeMode: function onArrangeMode() {
      if (this._focusScrollable !== this.folderScrollable) {
        this.cleanFolderScrollable();
      }
    },

    createCardNode: function createCardNode(card) {
    ...
    _createCardList: function _createCardList(cardList) {
      cardList.forEach((function (card) {
        this.cardListElem.appendChild(this.createCardNode(card));
      }).bind(this));
    },

    onMove: function onMove(key) {
    ...
    onEnter: function onEnter() {
      ...
      if (focusElem === this.settingsButton) {
        ...
        this.filterManager.resetFilter();
      } else if (focusElem && this.filterElementIds.indexOf(focusElem.id) > -1) {
        this.cleanFolderScrollable();
      } else {
    ...
    onSearchBarShown: function onSearchBarShown() {
    ...
    onSearchBarHidden: function onSearchBarHidden() {
      this.searchButton.classList.remove('hidden');
    },

    _getNavigateElements: function _getNavigateElements() {
      var elements = [];
    ...
    handleFocus: function handleFocus(elem) {
    ...
    handleUnfocus: function handleUnfocus(elem, nodeElem) {
      if (elem.CLASS_NAME == 'XScrollable') {
        this.handleCardUnfocus(elem, elem.currentItem, elem.getNodeFromItem(elem.currentItem));
      }
    },

    checkFocusedGroup: function checkFocusedGroup(elem) {
    ...
    handleFocusMenuGroup: function handleFocusMenuGroup(menuGroup) {
      var self = this;
    ...
    handleCardFocus: function handleCardFocus(scrollable, itemElem, nodeElem) {
      ...
      if (scrollable === this.cardScrollable && this._folderCard && itemElem.dataset.cardId !== this._folderCard.cardId && !this.cardScrollable.isHovering) {
        this.cleanFolderScrollable();
      }
    },

    cleanFolderScrollable: function cleanFolderScrollable(doNotChangeFocus) {
      if (this._focusScrollable === this.folderScrollable && !doNotChangeFocus) {
    ...
    handleCardUnfocus: function handleCardUnfocus(scrollable, itemElem, nodeElem) {
    ...
    handleCardUnhover: function handleCardUnhover(scrollable, itemElem, nodeElem) {
      this.cleanFolderScrollable();
    },

    determineFolderExpand: function determineFolderExpand(evt) {
      ...
      // transition.
      if (this.focusScrollable === this.cardScrollable && evt.originalTarget.classList.contains('app-button') && (!this._folderCard || this._folderCard.cardId !== evt.originalTarget.dataset.cardId) && (evt.originalTarget.classList.contains('focused') &&
      ...
      // to card.
      evt.propertyName === 'outline-width' && document.getElementById('main-section').dataset.mode !== 'arrange' ||
    ...
    buildFolderList: function buildFolderList(target) {
      ...
      var initFolderAnimation = (function () {
        if (isFirstFrame) {
          ...
          this.skipFolderBubble = Animations.doBubbleAnimation(this.folderListElem, '.app-button', 100, (function () {
    ...
    updateClock: function updateClock() {
      ...
      var timeFormat = use12Hour ? _('shortTimeFormat12') : _('shortTimeFormat24');
