				...
				if (mode == "navigation") {
					...
					if (level == 1) {
						if ($('.appbutton[data-number=' + parseInt(stage) + ']').attr('data-type') != "folder") {
							...
							if ($('.appbutton[data-number=' + parseInt(stage) + ']').attr('data-filter') == "web") circle_transition("#000", true);else circle_transition($('.appbutton[data-number=' + parseInt(stage) + ']').css('background-color'), true);
						...
						if (setting_stage == 2) {
							...
							TweenMax.delayedCall(timeout / 1000, function () {
								...
								TweenLite.to($('#setting'), .2, { backgroundColor: '#FFF', scaleX: 1.1, scaleY: 1.1, scaleZ: 1.1, z: 0.001, onComplete: function onComplete() {
										...
										TweenLite.to($('#dummy'), .4, { opacity: 1, zIndex: 100, ease: Power4.easeIn, delay: 0, onComplete: function onComplete() {
												flag = true;
												mode = "dummymode";
												playWave(false);
						...
						if ($('#addfolder-done p').hasClass('inactive')) console.log("inactive");else {
							...
							TweenLite.to($('#addfolder-done'), .3, { scaleX: 1, scaleY: 1, scaleZ: 1, z: 0.001, perspective: 1000, force3D: true, background: '#5eabd9', onComplete: function onComplete() {
									...
									TweenLite.to($('#addfolder'), .5, { opacity: 0, zIndex: -1, onComplete: function onComplete() {
											...
											if (position >= 5) {
												TweenLite.to($('.appbutton'), slide_speed, { left: '-=319', perspective: 1000, force3D: true, ease: Power1.easeInOut, onComplete: function onComplete() {
														//flag = true;
													} });
												position = 4;
												position_move += 1;
...
function circle_transition(c, b) {
	...
	var color = c;
	if (color == "rgba(0,0,0,0)") d = 200;else d = 1000;
