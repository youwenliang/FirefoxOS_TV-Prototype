	...
	(function ($) {
		$.fn.isSelected = function (s) {
			...
			if (this.hasClass('cardselected')) {
				...
				TweenLite.to(this, scale_speed, { border: '4px #FFF solid', borderTop: '4px #FFF solid', borderBottom: '4px #FFF solid', borderLeft: '4px #FFF solid', borderRight: '4px #FFF solid', scaleX: 1.6, scaleY: 1.6, scaleZ: 1.6, z: 0.001, zIndex: 10, force3D: true, ease: Power4.easeInOut, perspective: 1000, onComplete: function onComplete() {
						flag = true;
					} });
			} else {
										...
										if (currentposition != 1) {
											...
											if ($current.attr('data-type') == "card" && $next.attr('data-type') == "folder") {
												...
												$('#' + $next.attr('id') + ' > .appbutton-folder').each(function (index, element) {
													var k = $(this).attr('data-number').split('-')[1] - 1;
													TweenLite.to(this, speed3, { opacity: 1, width: origin_cardsize, height: origin_cardsize, marginLeft: '-=' + (30 - 58 * k) + 'px', bottom: '-300px', force3D: true });
												});
