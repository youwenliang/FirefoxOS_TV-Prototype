				...
				if (mode == "navigation") {
					...
					if (level == 1) {
						if ($('.appbutton[data-number=' + parseInt(stage) + ']').attr('data-type') != "folder") {
							...
							$('.appbutton[data-number=' + parseInt(stage) + ']').addClass('pressing');
							TweenMax.delayedCall(.2, function () {
								$('.appbutton[data-number=' + parseInt(stage) + ']').removeClass('pressing');
							});

							TweenLite.to($('.transition'), trans_speed, { opacity: 1, perspective: 1000, force3D: true, ease: Power4.easeOut, delay: .3 });
							TweenMax.delayedCall(1.5, function () {
								$('.appbutton[data-number=' + parseInt(stage) + ']').removeClass('selected');
							});
					...
					if (option_stage == 1) {
						...
						if (position_move > 0) {
							TweenLite.to($('#app'), slide_speed, { left: '+=' + 350 * position_move, perspective: 1000, force3D: true, ease: Power1.easeInOut, delay: .5 });
						}
										...
										TweenMax.delayedCall(.4, function () {
											if (stage != 1) {
												...
												if (position_move == 0) {
													position--;
													if (count <= 5) {
														TweenLite.to($('#app'), slide_speed, { marginLeft: '+=' + 158, perspective: 1000, force3D: true, ease: Power1.easeInOut });
													}
												} else if (position_move == count - 5) {
													position_move--;
													TweenLite.to($('#app'), slide_speed, { left: '+=' + 350, perspective: 1000, force3D: true, ease: Power1.easeInOut });
												} else {
					...
					TweenMax.delayedCall(.3, function () {
						...
						if (stage > 5) {
							TweenLite.to($('#app'), slide_speed, { left: '-=' + 350 * (stage - 5), perspective: 1000, force3D: true, ease: Power1.easeInOut, delay: .5 });
							position = 4;
						...
						if ($('#addfolder-done p').hasClass('inactive')) console.log("inactive");else {
							...
							TweenLite.to($('#addfolder-done'), .3, { scaleX: 1, scaleY: 1, scaleZ: 1, z: 0.001, perspective: 1000, force3D: true, background: '#5eabd9', onComplete: function onComplete() {
									...
									TweenLite.to($('#addfolder'), .5, { opacity: 0, zIndex: -1, onComplete: function onComplete() {
											...
											if (position >= 5) {
												TweenLite.to($('#app'), slide_speed, { left: '-=350', perspective: 1000, force3D: true, ease: Power1.easeInOut, onComplete: function onComplete() {
														//flag = true;
													} });
												position = 4;
												position_move += 1;
							...
							if (flag) {
								if (mode == "rearrange") {
									if (currentposition != 1) {
										...
										if ($current.attr('data-type') == "card" && $next.attr('data-type') == "folder") {
											...
											$('#' + $next.attr('id') + ' > .appbutton-folder').each(function (index, element) {
												var k = $(this).attr('data-number').split('-')[1] - 1;
												TweenLite.to(this, speed3, { opacity: 1, width: "180px", height: "180px", marginLeft: '-=' + (30 - 58 * k) + 'px', bottom: '-300px', perspective: 1000, force3D: true });
											});
									...
									if (rearrangelevel == 1) {
										...
										$('#' + $next.attr('id') + ' > .appbutton-folder').each(function (index, element) {
											TweenLite.to(this, speed3, { opacity: 0, width: notSelected_cardsize, height: notSelected_cardsize, marginLeft: '+=' + (30 - 58 * index) + 'px', bottom: '-80px', perspective: 1000, force3D: true });
										});
...
(function ($) {
	$.fn.closing = function () {
		...
		if (position_move > 0) {
			TweenLite.to($('#app'), slide_speed, { left: '+=' + 350 * position_move, ease: Power1.easeInOut, perspective: 1000, force3D: true, onComplete: function onComplete() {
					$backup = $('#app > .appbutton').clone().addClass('hide').removeClass('up');
