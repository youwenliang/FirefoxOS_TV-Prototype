...
(function (exports) {
  var EDIT_MODE_SCALE = 0.57;
  var CARD_TRANSFORM_LATENCY = 500;
  ...
  Edit.prototype = evt(Object.defineProperties({
    ...
    init: function init(spatialNavigator, cardManager, cardScrollable, folderScrollable) {
      var that = this;
      ...
      this.regularNavElements = [this.searchButton, this.settingsButton, this.editButton];
      ...
      this.cardScrollable.on('listTransformEnd', this.handleListTransformEnd.bind(this));
      this.cardScrollable.on('nodeTransformEnd', this.onCardMoveAnimationEnd.bind(this));
      this.cardScrollable.on('hovering-node-removed', this.onHoveringNodeRemoved.bind(this));
      this.cardScrollable.on('node-inserted-over-folder', this.onNodeInsertedOverFolder.bind(this));
    ...
    toggleEditMode: function toggleEditMode() {
      if (this.mainSection.dataset.mode === 'edit') {
        ...
        this.cardManager.writeCardlistInCardStore({ cleanEmptyFolder: true }).then((function () {
        ...
        this.currentNode = this.cardScrollable.getNodeFromItem(this.cardScrollable.currentItem);
    ...
    toggleArrangeMode: function toggleArrangeMode() {
      if (this.mainSection.dataset.mode === 'edit') {
        ...
        this.currentNode = this.currentScrollable.getNodeFromItem(this.currentScrollable.currentItem);
        this._revealPanel(this.currentScrollable, this.currentNode);
    ...
    _produceMove: function _produceMove(key) {
    ...
    _consumeMove: function _consumeMove() {
      ...
      if (!this._moveQueue.isEmpty() && focus.CLASS_NAME === 'XScrollable') {
        ...
        switch (key) {
          case 'left':
          case 'right':
            if (!focus.isHovering && targetItem && focus.currentItem.getAttribute('app-type') !== 'deck' && focus.currentItem.getAttribute('app-type') !== 'folder' && targetItem.getAttribute('app-type') === 'folder') {
              // hover on folder
              this.hoverCard(focus, focus.currentItem, targetItem);
              this._moveTimer = window.setTimeout(this.onCardMoveAnimationEnd.bind(this), CARD_TRANSFORM_LATENCY, targetItem);
              ...
              var focusedCard = this.cardManager.findCardFromCardList({ cardId: focus.currentItem.dataset.cardId });
              var nonFocusedCard = this.cardManager.findCardFromCardList({ cardId: targetItem.dataset.cardId });
              if (this.currentScrollable === this.folderScrollable) {
                var containingFolder = this.cardManager.findContainingFolder({ cardId: focus.currentItem.dataset.cardId });
          ...
          case 'down':
            if (focus.isHovering && this.isFolderReady) {
              this.moveToFolder();
              this._moveTimer = window.setTimeout(this.onCardMoveAnimationEnd.bind(this), CARD_TRANSFORM_LATENCY);
            }
    ...
    onCardMoveAnimationEnd: function onCardMoveAnimationEnd(elem) {
      if (this._moveTimer) {
        window.clearTimeout(this._moveTimer);
        this._moveTimer = undefined;
      }
      if (elem && !elem.classList.contains('focused') && !this._moveQueue.isEmpty()) {
        this._consumeMove();
      }
    },

    onCardSwapped: function onCardSwapped(card1, card2, idx1, idx2) {
    ...
    _setHintArrow: function _setHintArrow(scrollable) {
      ...
      if (scrollable && scrollable.isHovering) {
        ...
        this.currentNode.classList.toggle('right_arrow', index < this.currentScrollable.length - 1);
        this.currentNode.classList.remove('down_arrow');
        this.currentNode.classList.toggle('up_arrow', this.currentScrollable === this.folderScrollable);
      }
    },

    _setCurrentScrollable: function _setCurrentScrollable(scrollable) {
      this.currentScrollable = scrollable;
      document.getElementById('card-viewer').classList.toggle('current-scrollable', scrollable === this.cardScrollable);
      document.getElementById('folder-viewer').classList.toggle('current-scrollable', scrollable === this.folderScrollable);
    },

    _clearHintArrow: function _clearHintArrow() {
    ...
    addNewFolder: function addNewFolder() {
      var folder = this.cardManager.insertNewFolder({ id: 'new-folder' }, this.cardScrollable.currentIndex);
     ...
     */
    onHoveringNodeRemoved: function onHoveringNodeRemoved(cardItem, folderItem) {
      var card = this._hoveringCard;
      var folder = this.cardManager.findCardFromCardList({ cardId: folderItem.dataset.cardId });
      ...
      this.folderScrollable.realignToReferenceElement();
      this.currentNode = this.folderScrollable.getNodeFromItem(this.folderScrollable.currentItem);
    ...
    moveToCardList: function moveToCardList(focus) {
      var folder = this.cardManager.findContainingFolder({ cardId: focus.currentItem.dataset.cardId });
      var card = this.cardManager.findCardFromCardList({ cardId: focus.currentItem.dataset.cardId });
    ...
    onNodeInsertedOverFolder: function onNodeInsertedOverFolder() {
      this.spatialNavigator.focus(this.cardScrollable);
      this._setCurrentScrollable(this.cardScrollable);
      this.currentNode = this.cardScrollable.getNodeFromItem(this.cardScrollable.currentItem);
    ...
    renameCard: function renameCard(scrollable, nodeElem) {
    ...
    openDeleteCardDialog: function openDeleteCardDialog(scrollable, nodeElem) {
    ...
    deleteCard: function deleteCard(scrollable, nodeElem) {
      ...
      if (scrollable === this.folderScrollable) {
        var folder = this.cardManager.findContainingFolder({ cardId: itemElem.dataset.cardId });
        var card = this.cardManager.findCardFromCardList({ cardId: itemElem.dataset.cardId });
      }
    ...
    hoverCard: function hoverCard(scrollable, focusedItem, targetItem) {
      var card = this.cardManager.findCardFromCardList({ cardId: focusedItem.dataset.cardId });
      this._hoveringCard = card;
    ...
    unhoverCard: function unhoverCard(scrollable, shouldResetCardPositions) {
    ...
    _getPanel: function _getPanel(nodeElem) {
    ...
    _revealPanel: function _revealPanel(scrollable, nodeElem) {
      var panel = this._getPanel(nodeElem);
      if (scrollable.getItemFromNode(nodeElem).getAttribute('app-type') === 'deck') {
    ...
    _concealPanel: function _concealPanel(scrollable, nodeElem) {
      var panel = this._getPanel(nodeElem);
    ...
    handleFocus: function handleFocus(elem) {
      if (this.mode === 'edit' && elem.CLASS_NAME === 'XScrollable') {
        this.handleCardFocus(elem, elem.currentItem, elem.getNodeFromItem(elem.currentItem));
      }
    },

    handleUnfocus: function handleUnfocus(elem) {
      if (this.mode === 'edit' && elem === this.currentScrollable) {
        this._concealPanel(this.currentScrollable, this.currentNode);
        this.currentScrollable.spatialNavigator.focusSilently(this.currentScrollable.getItemFromNode(this.currentNode));
      }
    },
    ...
    handleListTransformEnd: function handleListTransformEnd() {
      if (this.mode === '') {
