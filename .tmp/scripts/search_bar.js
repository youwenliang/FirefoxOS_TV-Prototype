...
(function (exports) {
  ...
  proto.init = function sb_init(ui) {
    ...
    this.state = this.searchUI.classList.contains('hidden') ? SearchBar.STATES.HIDDEN : SearchBar.STATES.SHOWN;
  };
  ...
  proto.handleEvent = function sb_handleEvent(evt) {
    ...
    if (evt.target === this.searchUI && evt.propertyName === 'background-color' && this.state === SearchBar.STATES.SHOWING) {
      this.searchUI.classList.add('opened');
    } else if (evt.target === this.voiceButton && evt.propertyName === 'opacity' && this.state === SearchBar.STATES.SHOWING) {
