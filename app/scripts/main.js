					...
					if(level == 1){
						if($('.appbutton[data-number='+(parseInt(stage))+']').attr('data-type') != "folder"){
							...
							$('#dummy p').text('');
	  						...
	  						if($('.appbutton[data-number='+(parseInt(stage))+']').attr('data-filter')=="web") circle_transition("#000", true);
						...
						if ($('#addfolder-done p').hasClass('inactive')) console.log("inactive");
							...
							TweenLite.to($('#addfolder-done'), .3, {scaleX:1, scaleY:1, scaleZ:1, z: 0.001, perspective: 1000, force3D: true,   background: '#5eabd9', onComplete: function(){
								...
								TweenLite.to($('#addfolder'), .5, {opacity: 0, zIndex: -1, onComplete: function(){
									...
									if(position >= 5){
										TweenLite.to($('.appbutton'),slide_speed,{left: '-=319', perspective: 1000, force3D: true,   ease: Power1.easeInOut, onComplete: function(){
											//flag = true;
										}});
										position = 4;
										position_move += 1;
...
(function( $ ){
	$.fn.closing = function(){
		...
		if(position_move>0){
			TweenLite.to($('.appbutton'),slide_speed,{left: '+='+(319*position_move), ease: Power1.easeInOut, perspective: 1000, force3D: true,   onComplete: function(){
